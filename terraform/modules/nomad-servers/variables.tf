...
variable "name" {
  description = "The name of the server group, that will be used for naming resources"
  default     = "server"
}
...
variable "allowed_client_sgs" {
  description = "List of security groups for clients that are allowed to connect"
  type        = list
  default     = []
}

variable "nomad_port_http" {
  description = "HTTP port number to use for the nomad dashboard"
  default     = 4646
}

variable "consul_port_http" {
  description = "HTTP port number to use for the consul dashboard"
  default     = 8500
}

variable "node_gc_threshold" {
  description = "How long to keep dead node data before collecting"
  default     = "24h"
}

variable "job_gc_threshold" {
  description = "The interval between the job garbage collections"
  default     = "4h"
}
